...
# Dropdown for selecting multiple predefined datasets
st.subheader("Select one or more predefined datasets")
selected_datasets = st.multiselect("Choose Datasets", list(datasets.keys()))

# Upload one or more custom CSV files for dataset analysis
uploaded_files = st.file_uploader("Or upload one or more CSV files for analysis", type="csv", accept_multiple_files=True)

# Load selected datasets and uploaded files into a dictionary
loaded_datasets = {name: load_csv_dataset(datasets[name]) for name in selected_datasets}
if uploaded_files:
    loaded_datasets.update({file.name: pd.read_csv(file) for file in uploaded_files})
...
if len(loaded_datasets) == 1:
    # Single dataset from selection
    df = next(iter(loaded_datasets.values()))
    st.write("Selected Dataset:", list(loaded_datasets.keys())[0])
