...
with tab2:
    ...
    if 'df' in st.session_state and st.session_state.df is not None:
        ...
        def is_value_increasing(df):
            df_avg = df.groupby('Time')['value'].mean()
            return df_avg.iloc[-1] > df_avg.iloc[0]
    
        # Plot the data
        user_input = st.text_input("Type your message here",placeholder="Ask me about your data")
        if user_input:
            if "is the average value increasing" in user_input.lower():
                is_increasing = is_value_increasing(st.session_state.df)
                st.write("The average value is increasing over time." if is_increasing else "The average value is not increasing over time.")
            else:
                prompt_input = (
